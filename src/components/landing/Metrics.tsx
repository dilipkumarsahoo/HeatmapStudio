...
const stats = [
  ...
  },
  {
    icon: Clock,
    value: "120 hrs",
    label: "Average time saved per year",
  ...
  <section className="bg-background py-20">
    <div className="max-w-[1100px] mx-auto px-6 text-center animate-in fade-in slide-in-from-bottom-10 duration-1000">
      ...
      <h2 className="font-display text-4xl md:text-5xl font-medium text-foreground tracking-tight">
        Achieving Superior
        <br />
        <span className="text-foreground/30 italic">Efficiency Marks</span>
      </h2>
      <div className="mt-5 inline-flex flex-col sm:flex-row items-center gap-2 bg-foreground text-background text-[10px] sm:text-xs px-4 py-2 rounded-2xl sm:rounded-full max-w-full">
        <div className="flex items-center gap-2">
          <span className="bg-brand-yellow text-foreground text-[9px] font-bold px-1.5 py-0.5 rounded">
            ★
          </span>
          <span>
            HeatMap Studios is ranked <strong>#1</strong> personal toolkit
          </span>
        </div>
      </div>
      <div className="mt-12 grid grid-cols-2 md:grid-cols-4 gap-4">
        {stats.map((s) => (
          <div
            key={s.value}
            className="border border-foreground/10 rounded-2xl p-6 text-left bg-background"
          >
            <div
